...
export function requestLoadYouTubeSource(detail: YouTubeSourceDetail) {
  try {
    window.localStorage.setItem(LAST_YOUTUBE_URL_KEY, detail.url)
  } catch {
    // The current page interaction can still continue if storage is unavailable.
  }
  ...
  if (input) {
    ...
    if (setter) setter.call(input, detail.url)
    else input.value = detail.url
    input.dispatchEvent(new Event('input', { bubbles: true }))
    input.dispatchEvent(new Event('change', { bubbles: true }))
    window.setTimeout(() => input.closest('form')?.requestSubmit(), 0)
  }
