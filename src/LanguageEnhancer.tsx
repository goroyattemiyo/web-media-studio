...
function translateQuickControls(language: Language) {
  ...
  if (shuffle) {
    const next = language === 'ja' ? '🔀 シャッフル' : '🔀 Shuffle'
    if (shuffle.textContent !== next) shuffle.textContent = next
  }

  if (repeat) {
    const current = repeat.textContent?.toLowerCase() ?? ''
    const mode = current.includes('repeat 1') || current.includes('リピート 1') || current.includes('1曲')
      ? 'one'
      : current.includes('all') || current.includes('全曲')
        ? 'all'
        : 'off'
    const next = language === 'ja'
      ? `🔁 ${mode === 'one' ? '1曲リピート' : mode === 'all' ? '全曲リピート' : 'リピートOFF'}`
      : `🔁 ${mode === 'one' ? 'Repeat 1' : mode === 'all' ? 'Repeat All' : 'Repeat Off'}`
    if (repeat.textContent !== next) repeat.textContent = next
  }
}
...
function translateInterface(language: Language) {
  ...
  if (recorderModes[0]) {
    const strong = recorderModes[0].querySelector('strong'); const span = recorderModes[0].querySelector('span')
  ...
  if (ffmpegRun && !ffmpegRun.disabled) ffmpegRun.textContent = ja ? '音声化する' : 'Convert to audio'

  setText('.device-panel .section-heading h2', ja ? 'この端末で使える機能' : 'Browser capabilities')

  setDirectText('.appearance-trigger', ja ? '背景' : 'Backdrop')
